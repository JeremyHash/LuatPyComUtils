...
int checkid(DIR *dirp)
{
    ...
    while ((entp = readdir(dirp)) != NULL)
    {
        ...
        {
            if (ismatched(vender, entp->d_name) || ismatched("1782", entp->d_name))
                step--;
        }
        if (strcmp("idProduct", entp->d_name) == 0)
        {
            if (ismatched(product, entp->d_name) || ismatched("4d11", entp->d_name) || ismatched("4e00", entp->d_name))
                step--;
        }
        if (!step)
        {
...
char *findent(DIR *dirp, const char *regex[], int len)
{
    char *ret = NULL;
    regex_t *preg;
    int i;
    struct dirent *entp = NULL;
    preg = malloc(sizeof(regex_t) * len);
    if (!preg)
    {
        EXIT("malloc");
    }
    for (i = 0; i < len; ++i)
    {
        memset(preg + i, 0, sizeof(regex_t));
        regcomp(preg + i, regex[i], REG_EXTENDED | REG_NOSUB);
    }
    while ((entp = readdir(dirp)) != NULL)
    {
        for (i = 0; i < len; ++i)
        {
            if (regexec(preg + i, entp->d_name, 0, NULL, REG_NOTBOL | REG_NOTEOL) == 0)
            {
                debug("find %s\n", entp->d_name);
                ret = entp->d_name;
                goto end;
...
char *findif(DIR *dirp, const char *regex, const char *name)
{
    char *ifdir = NULL;
    char *dev = NULL;
    ifdir = findent(dirp, &regex, 1);
    if (ifdir != NULL)
    {
        DIR *dp = Opendir(ifdir);
        dev = findent(dp, &name, 1);
        if (strcmp(name, "net") == 0 && dev != NULL)
        {
            DIR *net = Opendir(dev);
            const char *_name[] = {"eth", "usb", "enx"};
            dev = findent(net, _name, 3);
            Closedir(net);
        }
        Closedir(dp);
    }
